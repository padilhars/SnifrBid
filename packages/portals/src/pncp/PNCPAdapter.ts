...
import type { PNCPListResponse, PNCPContratacao, PNCPDocumento } from './pncp.types.js';
...
export class PNCPAdapter extends BaseAdapter {
  ...
  async *fetchLicitacoes(
    ...
    for (const modalidadeCode of modalidadeCodes) {
      for (const ufCode of ufCodes) {
        ...
        while (true) {
          const response = await this.get<PNCPListResponse>('/contratacoes/publicacao', {
            dataInicial,
            dataFinal,
  ...
  async fetchDetalhes(externalId: string): Promise<LicitacaoColetada> {
    ...
    if (parts.length !== 3) throw new Error(`externalId inválido para PNCP: ${externalId}`);
    const [cnpj, ano, sequencial] = parts;
    const item = await this.get<PNCPContratacao>(`/contratacoes/${cnpj}/${ano}/${sequencial}`);

    // Buscar documentos
    let documentos: PNCPDocumento[] = [];
    try {
      const docsRes = await this.get<{ data: PNCPDocumento[] }>(
        `/contratacoes/${cnpj}/${ano}/${sequencial}/documentos`,
      );
